...
fdescribe ('Checkout Service test', () => {
  beforeEach(() => {
    ...
    sessionService = new SessionService(new TranslationsService(appSettings, document), appSettings, allergensService, basketService, promosService, document);
    statusService = new StatusService(appSettings, translatePipe);
    statusService.enabledPayments.push({
    ...
    instance = new CheckoutService(router, appSettings, sessionService, basketService, statusService, dynamicContentService, new PosOperationsService(appSettings));
  ...
  it('should check basket modification, check UUID, quantity', () => {
    basketService.buttons.push(new DotButton({
      Link: '1',
      ButtonType: 1,
    ...
    instance['_basketButtons'] = [];
    instance['_basketButtons'].push(new DotButton({
      Link: '1',
      ButtonType: 1,
      uuid: 'a4eda6d6-fb7a-11eb-9a03-0242ac130003',
      quantity: 2
  ...
  it('should return payment type', () => {
    const navigateSpy = spyOn(router, 'navigate');
    instance.startCheckoutTunnel();
    const paymentEnabled = statusService.enabledPayments.find(payment => payment.PaymentIsEnabled === true);
