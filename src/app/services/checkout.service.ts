...
import { PosElogHandler, DotButton, PosRefintService } from 'dotsdk';
...
export class CheckoutService {
  ...
  public async startCheckoutTunnel() {
    ...
    const paymentEnabled = this.statusService.enabledPayments.find(payment => payment.PaymentIsEnabled === true);
  ...
  protected async populateElogBeforeEndScene() {
    PosElogHandler.getInstance().posConfig.posHeader.amounts = {
      ...
      amountsTotalPaid: this.basketService.buttons.some(button => button['$$OrderDiscount']) && this.subtotalCents + this.taxCents >= this.orderDiscount() ?
  ...
  private sameBasket(): boolean {
    ...
    for (const oldButton of this.basketButtons) {
      const newButton = this.basketService.buttons.find(newBtn => newBtn.Link === oldButton.Link && newBtn.uuid === oldButton.uuid && newBtn.quantity === oldButton.quantity);
      if (!newButton) {
        return false;
      } else {
