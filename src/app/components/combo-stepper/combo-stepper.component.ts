...
import {
  enabledTouchlessMode,
  getTouchlessClass,
  isAdaEnabled,
} from '../../helpers/ada.helper';
...
import { PromoDiscountsMode } from '../../enums/promos.enum';
...
import { ComboStepperService } from 'src/app/services/combo-stepper.service';
...
export class ComboStepperComponent
  ...
  public isButtonChanged = false; // ! CHECK IF WE NEED THIS PROPERTY, USED IN VIEW (added by Sorin V)
  private _scrollIncrement;

  public get parentButton(): DotButton {
    const comboCatalog = FilesLoaderService.getInstance().loadersMap.get(
      CombosCatalog
    ) as CombosCatalog;
    return comboCatalog.Buttons.find(
      (btn) => btn.Link === this.button.ComboPage.ID.toString()
    );
  }

  public get comboStepButtons(): DotButton[] {
    if (
      this.interfaceCombos[
        this.currentComboStepIndex
      ].ComboStepType.toLocaleLowerCase() === ComboStepType.ComboUpSize.toLowerCase()
    ) {
      return this.interfaceCombos[this.currentComboStepIndex].Buttons.filter(
        (btn) =>
          Math.abs(Number(btn.Link)) >= Math.abs(Number(this.initialSize)) &&
          this.checkSizeHasReplacings(btn.Link)
      );
    }
    return this.interfaceCombos[this.currentComboStepIndex].Buttons.filter(
      (btn) =>
        btn.VisibleOn === this.currentSize ||
        (btn.ButtonType === DotButtonType.ITEM_PACK_BUTTON &&
          btn.Page?.Buttons?.length > 0)
    );
  }
  ...
  public get subtitle(): string {
    ...
    return upsizeButton
      ? this.dotTranslateCaption.transform(upsizeButton)
      : this.dotTranslateCaption.transform(this.button);
  }
  ...
  public get disableConfirmButton(): boolean {
    ...
    for (const modifier of this.modifiers) {
      const qtyButtons = modifier?.Buttons.reduce(
        (totalQuantity: number, button: DotButton) =>
          totalQuantity + button.quantity,
        0
      );
      //  disable step confirmation if modifier group MinQty not reached
      if (this.currentComboStepState === ComboStepState.CUSTOMIZE) {
        result =
          qtyButtons < modifier.PageInfo.MinQuantity &&
          !this.modifiers.some((x) =>
            x.Buttons.find((y) => y.AutoComplete === 1)
          );
      }
    }
    result =
      ...
      this.currentCombo.Buttons?.every(
        (btn) => !btn.Selected || btn.Page?.Buttons?.every((x) => !x.Selected)
      );
    return result;
  }
  ...
  public ngOnInit() {
    ...
    if (
      typeof this.button['$$forcePriceIn'] !== 'number' &&
      typeof this.button.ForcePriceIN === 'number'
    ) {
    ...
    this.currentSize =
      this.button.selectedSize ||
      this.button.StartSize ||
      this.button['DefaultSize'];
  ...
  public get modifiers(): DotModifier[] {
    return this.currentComboStepButton?.hasModifiers
      ? this.currentComboStepButton.ModifiersPage.Modifiers
      : [];
  }
  ...
  public setInlineStyle(index: number, zIndex: number) {
    ...
    const decreasePosition =
      standardPosition - (index >= 7 ? Math.round(index / 3) : index) * 40;
    return index % 2 === 0
      ? { left: `${decreasePosition}rem`, zIndex }
      : { right: `${decreasePosition}rem`, zIndex };
  }
  ...
  public backClick() {
    ...
    if (this.currentComboStepState === ComboStepState.CHOOSE) {
      const currentCombo = this.isLastStep
       ? this.interfaceCombos[this.currentComboStepIndex - 1 ]
       : this.currentCombo;
      this.currentComboStepIndex--;
      this.currentComboStepButton = currentCombo.Buttons.find(
        (btn) => btn.Selected
      );

      if (
        currentCombo.Buttons.filter((btn) => btn.VisibleOn === this.currentSize)
          .length === 1 &&
        currentCombo.Buttons[0]?.ModifiersPage?.Modifiers.some((mod) =>
          isAutoPopFeatVisible(mod, !this.button.isChanged, false)
        )
      ) {
        this.currentComboStepState = ComboStepState.CUSTOMIZE;
      }
  ...
  public cancelClick(): void {
    const contentRef = this.dynamicContentService.openContent(
  ...
  public checkAutoSelect(comboStepButtons: DotButton[]) {
    if (comboStepButtons.length === 1) {
      ...
      if (
        (!ModifiersPage ||
          !ModifiersPage?.Modifiers.some((mod) =>
            isAutoPopFeatVisible(mod, !this.button.isChanged, false)
          )) &&
        !this.isLastStep
      ) {
  ...
  public confirmClick() {
    ...
    if (this.interfaceCombos.length - 1 === this.currentComboStepIndex) {
      ...
      this.addComputedPrice(
        this.button['$$forcePriceIn'],
        this.button['$$forcePriceOut']
      );
      ...
      const suggestions =
        DotSuggestionSalesService.getInstance().getComboSuggestionsByLink(
          this.button.Link
        );
      if (
        suggestions &&
        ...
        this.button['parentLinkUUID'] =
          this.button['parentLinkUUID'] || generateUUID();
  ...
  public chooseComboButton(button: DotButton) {
    ...
    this.currentCombo.Buttons.forEach((btn) =>
      this.resetSubGroupAndButton(btn)
    );
    this.filteredComboStepButtons.forEach((btn) =>
      this.resetSubGroupAndButton(btn)
    );
    ...
    this.currentCombo.Buttons.forEach((btn) => {
      ...
      if (
        ...
        btn?.ModifiersPage?.Modifiers.forEach((x) => {
          const autoCompleteButton = x.Buttons.find(
            (mod) => mod.AutoComplete === 1
          );
          const qtyButtons = x.Buttons.reduce(
            (totalQuantity: number, dotBtn: DotButton) =>
              totalQuantity + dotBtn.quantity,
            0
          );
          if (
            x.PageInfo.MinQuantity &&
            x.PageInfo.MinQuantity > qtyButtons &&
            !autoCompleteButton
          ) {
            x.PageInfo.AutoPopFeat = '1';
          }
          if (
            btn?.ModifiersPage?.Modifiers.some((mod) =>
              isAutoPopFeatVisible(mod, !this.button.isChanged, false)
            )
          ) {
            this.verticalScrollChange();
  ...
  public getComboStepButtonPrice(button: DotButton) {
    ...
    const minPrice = this.currentCombo.Buttons.filter(
      (b) => b.VisibleOn === this.currentSize
    ).reduce((acc, btn) => {
    ...
    const newPrice =
      calculateButtonPrice(button, this.sessionService.serviceType) - minPrice;
    return newPrice >= 0 ? newPrice : 0;
  }
  protected nextStep() {
    if (
      this.currentCombo.ComboStepType.toLowerCase() === ComboStepType.ComboUpSize.toLowerCase() ||
      this.currentCombo.Buttons.filter(
        (btn) => btn.VisibleOn === this.currentSize
      ).length > 0
    ) {
      this.currentComboStepState = ComboStepState.CHOOSE;
    } else {
      this.currentCombo.Buttons[0].Selected = true;
      this.currentCombo.Buttons[0].quantity =
        this.currentCombo.Buttons[0].quantity || 1;
  ...
  protected modifierLabelTextFromComboCatalog() {
    this.parentButton?.ComboPage?.Combos?.forEach((combo) => {
      const comboCatalogResult = combo?.Buttons?.find(
        (btn) => btn.Link === this.currentComboStepButton.Link
      );
      if (comboCatalogResult) {
        this.comboModifierLabelText =
          this.translateComboModifierLabelPipe.transform(comboCatalogResult);
        return;
      }
    });
  }
  protected replaceItemsOnResize(newSize: string, button: DotButton) {
    button.ComboPage.Combos.filter(
      (c) => c.ComboStepType.toLowerCase() === ComboStepType.ComboStep.toLowerCase()
    ).forEach((combo) => {
  ...
  protected checkSizeHasReplacings(size: string) {
    return this.button.ComboPage.Combos.every((combo) =>
      combo.Buttons.some((btn) => btn.VisibleOn === size || btn.Link === size)
    );
  }
  ...
  protected setModifiersIncludedQuantity() {
    this.comboStepButtons.forEach((x) =>
      x.ModifiersPage?.Modifiers.forEach((modifier) => {
        if (
          Number.isInteger(modifier.PageInfo.MaxQuantity) &&
          modifier.PageInfo.MaxQuantity > 0
        ) {
          ...
          if (sumIncludedQuantities <= modifier.PageInfo.MaxQuantity) {
            modifier.Buttons.forEach((btn) => {
              btn.IncludedQuantity = btn.IncludedQuantity
                ? btn.IncludedQuantity
                : 0;
              btn.DefaultQuantity = btn.DefaultQuantity
                ? btn.DefaultQuantity
                : 0;
              if (
    ...
    this.comboStepButtons.forEach((x) =>
      x.ModifiersPage?.Modifiers?.forEach((modifier) => {
        modifier.Buttons.forEach((btn) => {
          if (
            btn.DefaultQuantity &&
            btn.DefaultQuantity > 0 &&
            !this.button.isChanged
          ) {
  ...
  protected addComputedPrice(buttonPriceInForced, buttonPriceOutForced): void {
    if (
      this.button.promoId &&
      this.sessionService.serviceType === PosServingLocation.IN &&
      typeof this.button.ForcePriceIN === 'number'
    ) {
    ...
    this.button.ComputedPrice = calculateButtonPrice(
      this.button,
      this.sessionService.serviceType
    );

    if (
      (typeof buttonPriceInForced !== 'number' &&
        this.sessionService.serviceType === PosServingLocation.IN) ||
      (typeof buttonPriceOutForced !== 'number' &&
        this.sessionService.serviceType === PosServingLocation.OUT)
    ) {
      return;
    }
    ...
    switch (this.appSettings.promoDiscountsMode) {
      ...
      case PromoDiscountsMode.EXCLUDE_PRICED_ITEMS:
        this.button.ForcePriceIN =
          this.button.ComputedPrice * this.button.quantity -
          this.button.MinPrice +
          buttonPriceInForced;
        this.button.ForcePriceOUT =
          this.button.ComputedPrice * this.button.quantity -
          this.button.MinPrice +
          buttonPriceOutForced;
      ...
      case PromoDiscountsMode.EXCLUDE_ONLY_PRICED_MODIFIERS:
        ...
        lButton.ComboPage?.Combos.forEach((combo) =>
          combo.Buttons.forEach((button) => (button.ModifiersPage = null))
        );
        const lButtonComputedPrice = calculateButtonPrice(
          lButton,
          this.sessionService.serviceType
        );
        if (
          ...
          this.button.ForcePriceIN =
            this.button.ComputedPrice -
            lButtonComputedPrice * this.button.quantity +
            buttonPriceInForced;
          this.button.ForcePriceOUT =
            this.button.ComputedPrice -
            lButtonComputedPrice * this.button.quantity +
  ...
  protected addAutoCompleteModifiers() {
    ...
    for (const modifier of this.modifiers) {
      const modifiersSelectedQuantity = modifier.selectedButtons.reduce(
        (acc, mod) => {
          acc += mod.quantity;
          return acc;
        },
      ...
      const autoCompleteModifierButton = modifier.Buttons.find(
        (y) => y.AutoComplete === 1
      );
      if (
        ...
        (!autoCompleteModifierButton.ComplementId ||
          (!this.hasComplementModifierSelected(
            autoCompleteModifierButton.ComplementId
          ) &&
            modifier.PageInfo.MinQuantity))
      ) {
        const modifierButton = modifier.Buttons.find(
          (buttonModifier) =>
            buttonModifier.Link === autoCompleteModifierButton.Link
        );
        if (modifierButton) {
  ...
  protected hasComplementModifierSelected(complementId: number): boolean {
    for (const modifier of this.modifiers) {
      if (
        modifier.Buttons.some(
          (buttonModifier) =>
            Number(buttonModifier.Link) === complementId &&
            buttonModifier.Selected
        )
      ) {
        return true;
      }
  ...
  private verticalScrollIncrement() {
    this._scrollIncrement = this.scrollRef
      ? this.scrollRef?.nativeElement?.clientHeight / 2
      : 0;
  }
  ...
  private filterSubgroupButtons(button: DotButton = null) {
    const currentComboStep = this.service.getCurrentComboStep(
      this.currentCombo.ComboStepID
    );

    if (button.ButtonType === DotButtonType.ITEM_PACK_BUTTON) {
      const currentComboButton = currentComboStep.Buttons.find(
      ...
      this.filteredComboStepButtons = currentComboButton.Page.Buttons.filter(
        (b) =>
          Math.abs(Number(b.VisibleOn)) === Math.abs(Number(this.currentSize))
      );
    }
  }
