...
import { DynamicContentService } from '../../services/dynamic-content';
...
import { LinkToParentStatus } from '../../enums/general.enum';
...
export class SuggestionSalesComponent extends AbstractDynamicComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
  ...
  constructor(private dataParams: DynamicContentParams,
  ...
  public onButtonClick(button: DotButton) {
    if (button.hasModifiers && button.ModifiersPage?.Modifiers.some(mod => isAutoPopFeatVisible(mod, true, true))) {
      const modifierRef = this.dynamicContentService.openContent(ButtonDetailsComponent, {btn: button, disableAddButtonToBasket: true, fromSuggestions: true });
      this.subscriptions.push(modifierRef.afterClosed.subscribe( btn => {
      ...
      const comboStepperRef = this.dynamicContentService.openContent(ComboStepperComponent, {btn: button, disableAddButtonToBasket: true, fromSuggestions: true});
      this.subscriptions.push(comboStepperRef.afterClosed.subscribe(btn => {
        if (btn.Selected) {
          const index = this.page.Buttons.findIndex(b => btn.Link === b.Link);
          if (index >= 0) {
            this.page.Buttons.splice(index, 1, btn);
  ...
  public quantityChanged(button: DotButton, changedQuantity?: number) {
    if (changedQuantity > 0) {
      if ((button.hasCombos || (button.hasModifiers && button.ModifiersPage?.Modifiers.some(mod => isAutoPopFeatVisible(mod, true, true)))) &&
        button.quantity <= 0) {
        this.onButtonClick(button);
      } else if ((this.page.MaxQty && this.totalQuantity < this.page.MaxQty) ||
        (!this.page.MaxQty && this.totalQuantity >= 0 )) {
        button.Selected = true;
        button.quantity++;
      }
