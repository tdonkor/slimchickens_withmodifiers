...
import { DotButton, DotPage, generateUUID, PromoNode } from 'dotsdk';
...
export class PromoStepperComponent extends AbstractDynamicComponent implements OnInit, OnDestroy {
  ...
  public selectPromoButton(button: DotButton) {
    ...
    if (button.Selected) {
      ...
      const comboStepperRef = this.dynamicContentService.openContent(ComboStepperComponent, {btn: button, disableAddButtonToBasket: true});
  ...
  private addSelectedButton(button: DotButton) {
    ...
    button.quantity = 1;
    this.selectedPromoButtons.push({step: this.currentStepIndex, button: button});
  }
}
