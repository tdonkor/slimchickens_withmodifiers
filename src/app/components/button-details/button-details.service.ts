...
export class ButtonDetailsService {
      ...
      if (Number.isInteger(modifier.PageInfo.MaxQuantity) && modifier.PageInfo.MaxQuantity > 0) {
        ...
        if (sumIncludedQuantities <= modifier.PageInfo.MaxQuantity) {
          modifier.Buttons.forEach(btn => {
            ...
            if (Number.isInteger(btn.MinQuantity) && btn.MinQuantity > 0 && Number.isInteger(btn.IncludedQuantity) && btn.IncludedQuantity === 0) {
  ...
  public addAutoCompleteModifiers() {
    ...
    for (const modifier of this.modifiers) {
      ...
      if (autoCompleteModifierButton && modifier.PageInfo.MinQuantity > modifiersSelectedQuantity && (!autoCompleteModifierButton.ComplementId || !this.hasComplementModifierSelected(autoCompleteModifierButton.ComplementId) && modifier.PageInfo.MinQuantity)) {
