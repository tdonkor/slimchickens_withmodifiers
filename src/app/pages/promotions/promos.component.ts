...
import { DotPage, DotButton, PromotionsService, DotButtonType, PromotionQuery, OrderDiscountResponse, PromoNode } from 'dotsdk';
import { PROMOS_STATE, PromoResponseType } from '../../enums/promos.enum';
...
import { ProductStatus, PromoInputOption } from '../../enums/general.enum';
...
export class PromosComponent extends AbstractDynamicComponent implements OnInit, OnDestroy {
  ...
  public async ngOnInit() {
    switch (this.appSettingsService.promoInputOption) {
      case PromoInputOption.BOTH:
        (this.statusService.scannerActive) ?
          this.componentState = PROMOS_STATE.START :
      ...
      if (Number.isInteger(modifier.PageInfo.MaxQuantity) && modifier.PageInfo.MaxQuantity > 0) {
        ...
        if (sumIncludedQuantities <= modifier.PageInfo.MaxQuantity) {
          modifier.Buttons.forEach(btn => {
            ...
            if (Number.isInteger(btn.IncludedQuantity) && btn.IncludedQuantity > 0 && Number.isInteger(btn.MaxQuantity) && btn.MaxQuantity > 0) {
              ...
              if (Number(btn.ButtonStatus) === ProductStatus.UNAVAILABLE && Number.isInteger(btn.IncludedQuantity) && btn.IncludedQuantity > 0) {
  ...
  protected usePromoCode(promoCode: string) {
    switch (this.promosService.findPromoByBarcode(promoCode)) {
      case PromoResponseType.ORDER_DISCOUNT:
        ...
        btn.Price = (this.promosService.orderDiscountPromoInfo.percent / 100).toString() + '% ' + this.translatePipe.transform('20210504001');
        ...
        const contentRef = this.dynamicContentService.openContent(InfoDialogComponent, {
          title: oldOrderDiscountButtonIndex >= 0 ?
            this.translatePipe.transform('20210504005') :
            this.translatePipe.transform('20210504002'),
  ...
  protected showScannerError(errorMessage: string) {
    ...
    this.subscriptions.push(contentRef.afterClosed.subscribe(response => {
      if (response === 'Yes') {
        ...
        this.componentState = this.appSettingsService.promoInputOption === PromoInputOption.BOTH ?
        PROMOS_STATE.START :
        PROMOS_STATE.SCAN;
      }
    }));
  ...
  private promoNumberAllowed(promoCode: string, promotionQuery: PromotionQuery): boolean {
    ...
    if ((promotionQuery.maximumPromotionNumber < 1 && (promotionQuery.promoPayload instanceof OrderDiscountResponse)) ||
    ...
    if ((promotionQuery.maximumAllowedPromotionsNumber <= this.promosService.scannedBarcodes.length &&
      ...
      (promotionQuery.maximumAllowedPromotionsNumber <= this.promosService.scannedBarcodes.length && (promotionQuery.promoPayload instanceof PromoNode))) {
