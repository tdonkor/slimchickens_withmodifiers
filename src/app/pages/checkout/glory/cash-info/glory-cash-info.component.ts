...
import { AtpPaymentService, AtpPrinterService, cancelOrderEvent, PosRefintService, PosPaidState, PosTenderType, PosElogHandler } from 'dotsdk';
...
export class GloryCashInfoComponent implements OnInit, OnDestroy {
  ...
  public async cashBack() {
    ...
    const cashBackResult = await AtpPaymentService.getInstance().cashBack(this.refInt, this.cashPaymentName)
                                                .catch(e => {
    ...
    if (cashBackResult.Error) {
      this.router.navigate(['glory-cash-info', CashInfoAction.PRINT]);
    } else if (cashBackResult.Response.ReturnedAmount < cashBackResult.Response.AmountToBeReturned ||
  ...
  public async overpayEndAcceptMoney() {
    ...
    if (endResult && !endResult.Error) {
      if (this.paymentModelService.getAmountOwed() > this.paymentModelService.getTotalAmountPaidWithCash() +
      this.paymentModelService.getCurrentAmountPaidWithCash() + this.paymentModelService.getTotalAmountPaidWithGift()) {
        this.text = { title: '2021022402', message: '2021022403', amount: this.returnedAmount};
        setTimeout(() => {
          this.paymentModelService.endCashTransaction();
          this.router.navigate(['payment-selection']);
        ...
        return;
      } else if (this.paymentModelService.getCashAmountRefunded() >= this.paymentModelService.getCurrentAmountPaidWithCash() - this.paymentModelService.getCashAmountToKeep()) {
        this.text = { title: '2021022402', message: '2021022403', amount: this.returnedAmount };
      } else {
